...
function getDatabaseUrl(): string {
  ...
  if (!v || v.includes("mysql://USER:PASSWORD@HOST:PORT/DATABASE")) {
    throw new Error(
      "DATABASE_URL is missing/placeholder. Put a real URL in .env (for CLI seed), and in Vercel env vars."
    );
  }
  return v;
}
...
function createSeedPrisma() {
  ...
  const adapter = new PrismaMariaDb(mysqlUrlToAdapterConfig(url));
...
async function main() {
  ...
  await prisma.orderCounter.create({ data: { id: 1, next: 37 } });

  await prisma.product.createMany({
    data: [
      { name: "Bulk Bag of Logs", description: "Seasoned hardwood bulk bag", pricePence: 17500, isActive: true, sortOrder: 1, imageAlt: "Bulk bag of logs", imageUrl: null },
      { name: "IBC Crate", description: "Seasoned hardwood in an IBC crate", pricePence: 19500, isActive: true, sortOrder: 2, imageAlt: "IBC crate of logs", imageUrl: null },
      { name: "Net of Logs", description: "Convenient net of seasoned logs", pricePence: 1000, isActive: true, sortOrder: 3, imageAlt: "Net of logs", imageUrl: null },
      { name: "Kindling Bag", description: "Dry kindling bag", pricePence: 600, isActive: true, sortOrder: 4, imageAlt: "Kindling", imageUrl: null },
    ],
  });
  ...
  const orderStatuses = ["NEW", "PAID", "OFD", "DELIVERED", "CANCELLED"] as const;
  const paymentStatuses = ["UNPAID", "PENDING", "PAID", "FAILED", "EXPIRED", "CANCELED"] as const;
  const methods = ["MOLLIE", "CASH", "BACS"] as const;
  ...
  for (let i = 1; i <= 36; i++) {
    ...
    if (status === "DELIVERED") {
      paymentStatus = pick(["PAID", "UNPAID"] as const);
    }
    ...
    if (status === "CANCELLED") {
      ...
      paymentStatus = pick(["UNPAID", "FAILED", "EXPIRED", "CANCELED"] as const);
      cancelReason = pick([
        "Customer cancelled",
    ...
    const molliePaymentId = checkoutPaymentMethod === "MOLLIE" ? `tr_seed_${i}_${randInt(1000, 9999)}` : null;
    const mollieCheckoutUrl = checkoutPaymentMethod === "MOLLIE" ? `https://checkout.mollie.com/pay/seed_${i}` : null;
    const paidAt = paymentStatus === "PAID" ? new Date(createdAt.getTime() + randInt(5, 180) * 60 * 1000) : null;
    ...
    const order = await prisma.order.create({
      data: {
        ...
        town: "Wincanton",
        county: "Somerset",
        addressLine1: pick(["1 High Street", "Rose Cottage", "The Old Mill", "Unit 3, Business Park", null]) ?? null,
        addressLine2: pick(["Verrington", "Lower Street", null]) ?? null,
        preferredDay: pick(["Mon", "Tue", "Wed", "Thu", "Fri", null]) ?? null,
        deliveryNotes: pick(["Leave by garage", "Call on arrival", "Gate code 1234", null]) ?? null,
